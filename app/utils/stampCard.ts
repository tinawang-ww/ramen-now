...
function loadImage(src: string) {
  return new Promise<HTMLImageElement | null>((resolve) => {
    ...
    // A missing picture degrades to an empty ring, not a failed card.
...
/**
 * Draws the stamp book as a shareable PNG — the same bowl art, rings and
 * milestone tiers as the page, so the flex travels intact.
 */
export async function renderStampBook(stamps: StampSummary[], text: StampCardText): Promise<Blob> {
  const drawn = stamps.slice(0, MAX_STAMPS)
  const rows = Math.max(1, Math.ceil(drawn.length / COLUMNS))

  const images = await Promise.all(drawn.map(stamp => loadImage(shopStampSrc(stamp.shopId))))
  ...
  drawn.forEach((stamp, index) => {
    ...
    const gold = stamp.reports >= 10
    const tone = gold ? MARIGOLD : TONES[stamp.shopId % TONES.length]!
    const image = images[index]

    // Same tilt rule as the board rows: keyed on the id, hand-stamped but stable.
    ctx.save()
    ctx.translate(cx, cy)
    ctx.rotate(((stamp.shopId % 7) - 3) * Math.PI / 180)

    // The bowl art, clipped into the chop.
    if (image) {
      ctx.save()
      ctx.beginPath()
      ctx.arc(0, 0, 82, 0, Math.PI * 2)
      ctx.clip()
      ctx.drawImage(image, -82, -82, 164, 164)
      ctx.restore()
    }

    ctx.strokeStyle = tone
    ctx.lineWidth = gold ? 9 : 7
    ctx.beginPath()
    ...
    // The ×N badge, bottom-right like the page.
    ctx.setLineDash([])
    ctx.fillStyle = PAPER
    ctx.strokeStyle = 'rgba(0, 1, 42, 0.12)'
    ctx.lineWidth = 2
    ctx.beginPath()
    ctx.arc(62, 62, 26, 0, Math.PI * 2)
    ctx.fill()
    ctx.stroke()
    ...
    ctx.fillText(`×${stamp.reports}`, 62, 63)
    ctx.restore()
    ...
    ctx.fillText(truncate(ctx, stamp.name, CELL - 16), cx, cy + 138)
    ctx.fillStyle = MIST
    ctx.font = `400 22px ${FONT}`
    ctx.fillText(stampDate(stamp.firstAt), cx, cy + 174)
    ...
    if (stamp.regular && text.regularLabel) {
      ...
      ctx.beginPath()
      ctx.roundRect(cx - width / 2, cy + 188, width, 30, 15)
      ctx.fill()
      ctx.fillStyle = 'rgba(0, 1, 42, 0.7)'
      ctx.fillText(text.regularLabel, cx, cy + 209)
    }
